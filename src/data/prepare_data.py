...
def prepare(input_filepath, output_filepath):
    ...
    input_filepath= click.prompt('Enter the file path for the input data', type=click.Path(exists=True), default="data/raw")
    input_filepath_admission = f"{input_filepath}/admission.csv"
    output_filepath = click.prompt('Enter the file path for the output preprocessed data (e.g., output/preprocessed_data.csv)', type=click.Path(),default="data/processed")
    
    # Call the main data processing function with the provided file paths
    process_data(input_filepath_admission, output_filepath)

def process_data(input_filepath_admission, output_folderpath):
    """
    ...
    """
    #--Importing dataset
    df_admission = pd.read_csv(input_filepath_admission, sep=",")

    if False: 
        #examples to consider or not
    ...
    if True:
        #--Dropping columns 
        list_to_drop = ['Serial No.' ]
        df_admission.drop(list_to_drop, axis=1, inplace=True)
    ...
    print(df_admission.isna().sum(), df_admission.info(),df_admission.describe(), df_admission.shape, df_admission.columns, sep="\n\n")

    target = df_admission['Chance_of_Admit']
    feats = df_admission.drop(['Chance_of_Admit'], axis = 1)
    ...
    if check_existing_folder(output_folderpath) :
        print(f"Creating {output_folderpath}")
        os.makedirs(output_folderpath)
    else:
        print(f"Existing {output_folderpath}")    
    ...
    for file, filename in zip([X_train, X_test, y_train, y_test], ['X_train', 'X_test', 'y_train', 'y_test']):
        ...
        if check_existing_file(output_filepath):
            ...
            print(f"Existing {output_filepath}")    
